...
func newLoadSimCollector(context context.Context, log logr.Logger, supportArchive tarball, fileSize int, fileCount int, pods clientgocorev1.PodInterface) collector { //nolint:revive // argument-limit doesn't apply to constructors
...
func (collector loadSimCollector) createSimulatedLogFiles() {
	for i := 0; i < collector.fileCount; i++ {
		...
		err := collector.supportArchive.addFileInParts(fileName, &lg)
